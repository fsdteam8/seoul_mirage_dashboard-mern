...
//         const res = await fetch(
//           productToEdit
//             ? `${process.env.NEXT_PUBLIC_API_URL}/api/products/${productToEdit.id}`
...
//       formData.append("status", data.arrival_status);
...
export function AddProductSheet({
  ...
  React.useEffect(() => {
    if (isOpen) {
      if (productToEdit) {
        ...
        // Set existing images and clear new uploads
        const imageUrls =
          productToEdit.media?.map((media) => media.file_path) || [];
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      if (existingImages.length > 0) {
        formData.append("existing_media", JSON.stringify(existingImages));
      }
  ...
  const removeImage = (indexToRemove: number) => {
    ...
    if (previewToRemove?.startsWith("blob:")) {
      ...
      URL.revokeObjectURL(previewToRemove);
      setSelectedFiles((prev) =>
        prev.filter((_, index) => index !== (indexToRemove - existingImages.length))
      );
    } else {
