...
export async function getPromoCodes(
  ...
  let filteredPromoCodes = promoCodesStore.map((pc) => ({
    ...pc,
    effectiveStatus: getEffectivePromoCodeStatus(pc),
  }));
  ...
  if (searchTerm) {
    ...
    filteredPromoCodes = filteredPromoCodes.filter(
      (pc) =>
        pc.name.toLowerCase().includes(lowerSearchTerm) ||
        (pc.description &&
          pc.description.toLowerCase().includes(lowerSearchTerm))
    );
  }

  if (statusFilter && statusFilter !== "All") {
    filteredPromoCodes = filteredPromoCodes.filter(
      (pc) => getEffectivePromoCodeStatus(pc) === statusFilter
    );
  }

  // Sort by creation date descending
  filteredPromoCodes.sort(
...
export async function addPromoCodeAction(
  ...
  if (
    promoCodesStore.some(
      (pc) => pc.name.toLowerCase() === promoCodeData.name.toLowerCase()
    )
  ) {
    return {
      success: false,
      message: `Promo code "${promoCodeData.name}" already exists.`,
    };
  }
...
export async function updatePromoCodeAction(
  promoCodeId: string,
  promoCodeData: Partial<
    Omit<PromoCode, "id" | "timesUsed" | "createdAt" | "updatedAt">
  >
): Promise<{ success: boolean; message: string; promoCode?: PromoCode }> {
  await new Promise((resolve) => setTimeout(resolve, 700));
  const promoCodeIndex = promoCodesStore.findIndex(
    (pc) => pc.id === promoCodeId
  );
  ...
  if (
    ...
    if (
      promoCodesStore.some(
        (pc) =>
          pc.id !== promoCodeId &&
          pc.name.toLowerCase() === promoCodeData.name?.toLowerCase()
      )
    ) {
      return {
        success: false,
  ...
  return {
    ...
    promoCode: promoCodesStore[promoCodeIndex],
  };
}
...
export async function generateUniquePromoCodeString(
  length = 8,
  prefix = ""
): Promise<string> {
  ...
  while (attempts < 100) {
    ...
    if (
      !promoCodesStore.some(
        (pc) => pc.name.toLowerCase() === result.toLowerCase()
      )
    ) {
      return result;
    }
