...
export type OrderFulfillmentStatus = "delivered" | "pending" | "cancelled";
...
const generateRandomItems = (): OrderItem[] => {
  ...
  for (let i = 0; i < itemCount; i++) {
    items.push({
      ...
      name: `Product ${String.fromCharCode(
...
export const mockOrders: Order[] = Array.from({ length: 25 }, (_, i) => {
  ...
  const paymentStatuses: OrderPaymentStatus[] = ["paid", "unpaid", "failed"];
